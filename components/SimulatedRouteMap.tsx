...
const SimulatedRouteMap: React.FC<SimulatedRouteMapProps> = ({ waypoints, optimizedRouteDescription }) => {
  ...
  const getPosition = (index: number, total: number) => {
    ...
    let yOffset = 0;
    if (total > 2) {
      const peakIndex = Math.floor(total / 3); 
      const valleyIndex = Math.floor(total * 2 / 3); 
      
      if (index <= peakIndex) {
        yOffset = -40 * (index / peakIndex); 
      } else if (index <= valleyIndex) {
        
        const progress = (index - peakIndex) / (valleyIndex - peakIndex);
        yOffset = -40 + 80 * progress; 
      } else {
        
        const progress = (index - valleyIndex) / (total - 1 - valleyIndex);
        yOffset = 40 - 40 * progress; 
      }
    }
    ...
    y = Math.max(padding + 10, Math.min(y, mapHeight - padding - 25)); // Adjusted for larger bottom label space
    ...
    <Card title={t('adminTruckPlanning_routeMap_title')} className="bg-slate-800 shadow-lg">
      <div className="p-4">
            ...
            {waypoints.map((waypoint, i) => {
              ...
              return (
                <g key={`waypoint-${i}`} transform={`translate(${x - 8}, ${y - 16})`}> 
                   ...
                   <text x="8" y="30" className="text-xs fill-slate-200" textAnchor="middle"> {/* Increased from text-[10px] */}
                     {waypoint.order + 1}. {waypoint.name.length > 18 ? `${waypoint.name.substring(0,15)}...` : waypoint.name}
                   </text>
                </g>
              );
            })}
        ...
        <div className="mt-3">
            ...
            <ul className="text-sm text-slate-300 space-y-1 max-h-28 overflow-y-auto custom-scrollbar pr-2"> {/* Increased from text-xs */}
                {waypoints.sort((a,b) => a.order - b.order).map(wp => ( 
