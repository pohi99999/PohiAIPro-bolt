...
import { ai } from '../../lib/gemini'; // Import shared AI instance
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <Card title={t('adminStock_aiProductCategorization')}>
          ...
          {state.categorySuggestion && !isLoadingAi.categorySuggestionOp && state.currentAiFeatureKey === 'categorySuggestionOp' && (
            <div className="mt-4 p-3 bg-slate-700/50 rounded">
              <h5 className={`font-semibold mb-1 ${typeof state.categorySuggestion === 'string' && (state.categorySuggestion.includes("Error") || state.categorySuggestion.includes("Hiba") || state.categorySuggestion.includes(t('adminStock_error_provideMinLengthDescription', {action:''}).substring(0,10)) ) ? "text-red-400" : "text-cyan-400"}`}>{t('adminStock_suggestedCategories')}</h5>
        ...
        <Card title={t('adminStock_aiSustainabilityAssessment')}>
          ...
          {state.sustainabilityAssessment && !isLoadingAi.sustainabilityAssessmentOp && state.currentAiFeatureKey === 'sustainabilityAssessmentOp' && (
            <div className="mt-4 p-3 bg-slate-700/50 rounded">
               <h5 className={`font-semibold mb-1 ${typeof state.sustainabilityAssessment === 'string' &&  (state.sustainabilityAssessment.includes("Error") || state.sustainabilityAssessment.includes("Hiba") || state.sustainabilityAssessment.includes(t('adminStock_error_provideMinLengthDescription', {action:''}).substring(0,10)) ) ? "text-red-400" : "text-cyan-400"}`}>{t('adminStock_aiAssessment')}</h5>
            ...
            <Card key={item.id} className="flex flex-col justify-between hover-glow transition-shadow duration-300">
              <div>
                ...
                <div className="p-4 space-y-3">
                  <div className="flex items-start text-sm text-slate-300">
                    ...
                    <span>
                      <span className="font-medium text-slate-100">{t('adminStock_stockItem_features')}:</span> {item.productName || item.diameterType}, Ø {item.diameterFrom}-{item.diameterTo}cm, {t('customerNewDemand_length').toLowerCase()}: {item.length}m, {item.quantity}pcs
                    </span>
                  </div>
              ...
              <div className="p-4 border-t border-slate-700 bg-slate-800/50">
                 ...
                 {state.selectedStockItemIdForAdminAi === item.id && state.adminAiStockAnalysisResult && !state.isAdminAiLoadingForStockItem && state.currentAiFeatureKey === `adminAnalysis-${item.id}` && (
                    <div className={`mt-3 p-3 rounded text-xs ${typeof state.adminAiStockAnalysisResult === 'string' && (state.adminAiStockAnalysisResult.includes("Error") || state.adminAiStockAnalysisResult.includes("Hiba")) ? 'bg-red-900/50 border border-red-700 text-red-300' : 'bg-slate-700 text-slate-200'}`}>
